# /home/mines/Documents/oldcast1e/MinesLab/IGEV-plusplus/src/run_test_imgs_edited.py
"""
실행 방법:
conda activate IGEV_plusplus
cd ~/Documents/oldcast1e/MinesLab/IGEV-plusplus
python src/run_test_imgs_edited.py
"""
...
# 2) Run demo_imgs.py
# ======================
before_files = set(glob.glob(os.path.join(OUTPUT_DIR, "*")))  # 실행 전 파일 목록
...
after_files = set(glob.glob(os.path.join(OUTPUT_DIR, "*")))   # 실행 후 파일 목록
new_files = list(after_files - before_files)  # 새로 생긴 파일만 잡기
print(f"[OK] Disparity estimation complete → {OUTPUT_DIR}")

# ======================
# 3) Rename 새 결과만 IMG_ID 기반으로 통일
# ======================
for f in new_files:
    ext = os.path.splitext(f)[1]
    new_name = os.path.join(OUTPUT_DIR, f"{IMG_ID}{ext}")
